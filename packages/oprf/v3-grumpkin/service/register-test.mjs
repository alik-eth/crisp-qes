...
import {
    ...
    NULLIFIER_M_X_WORD_INDEX,
    NULLIFIER_C_R_WORD_INDEX,
...
import {
    Fp, Fn, N, G, hashToCurve, oprfEval, dleqProve, scalarLimbs, commitR,
} from "../lib.mjs";
...
async function ensureNullifierProof() {
    ...
    // C_r = commit_r(r): must equal the enroll proof's C_r (cross-check (d), F2).
    const cR = await commitR(R_SCALAR);

    // New 8-word ABI: kpx,kpy, yx,yy, mx,my, c_r (public) + r,rinv,c,z (private).
    // No free G.x/G.y (F1 closed) and no c_expected (C-1 is the lib limb binding).
...
async function main() {
    ...
    check("nullifier proof M == enroll proof M",
        nullifierPublicInputs[NULLIFIER_M_X_WORD_INDEX] === Mx
            && nullifierPublicInputs[NULLIFIER_M_X_WORD_INDEX + 1] === My);
    // The nullifier proof's C_r == the enroll proof's C_r (cross-check (d), F2).
    check("nullifier proof C_r == enroll proof C_r",
        BigInt(nullifierPublicInputs[NULLIFIER_C_R_WORD_INDEX])
            === BigInt(enrollPublicInputs[C_R_WORD_INDEX]),
        `null=${nullifierPublicInputs[NULLIFIER_C_R_WORD_INDEX]} enroll=${enrollPublicInputs[C_R_WORD_INDEX]}`);
    ...
    // (j) MISMATCHED C_r is REJECTED via cross-check (d) — the F2 cross-proof
    // binding. Flip the nullifier proof's public C_r word so it != the enroll
    // proof's C_r; cross-check (d) runs BEFORE the wasm verify, so the request is
    // rejected as NullifierMismatchedCr (not merely a crypto failure).
    ...
    const badCrPublics = nullifierPublicInputs.slice();
    badCrPublics[NULLIFIER_C_R_WORD_INDEX] =
        bigintToHex32(BigInt(nullifierPublicInputs[NULLIFIER_C_R_WORD_INDEX]) + 1n);
    const badCrRes = await app3.inject({
        method: "POST",
        url: "/v3/register",
        payload: { ...validPayload, nullifierPublicInputs: badCrPublics },
    });
    check("register: mismatched C_r => 4xx (cross-proof r-binding enforced)",
        badCrRes.statusCode >= 400 && badCrRes.statusCode < 500,
        `status=${badCrRes.statusCode} body=${JSON.stringify(badCrRes.json()).slice(0, 160)}`);
    check("register: mismatched-C_r rejection code is NullifierMismatchedCr",
        badCrRes.json().error === "NullifierMismatchedCr",
        badCrRes.json().error);
    await app3.close();
