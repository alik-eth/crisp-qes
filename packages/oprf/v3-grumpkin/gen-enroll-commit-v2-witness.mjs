...
import { N, mapToCurveSvdW, hashToField2, scalarLimbs, commitR } from "./lib.mjs";
...
// --- OPRF / hash-to-curve material (ENROLL = the blind-eval GATE) ---
// The enroll circuit only BLINDS (M = r*H2C(id)) and commits to r (C_r); it does
// NOT do the DLEQ/unblind/nullifier (that is the separate oprf_nullifier proof,
// with its own generator). So here we need just r, the H2C hints, M, and C_r.
//   r = client blinding scalar (the SAME r the oprf_nullifier proof reuses).
...
// C_r = commitR(r): the circuit's PUBLIC output for the cross-proof shared-r
// binding (F2). The generator does not supply outputs, but we log it so the
// witness-shape reference shows the value the circuit will emit.
...
// --- scalar limbs for the enroll ABI (only r is a scalar input here) ---
...
const toml = `# auto-generated by gen-enroll-commit-v2-witness.mjs (v3 two-proof ENROLL gate + Diia chain)
# SYNTHETIC disposable cert + signedAttrs + synthetic test CA. Never a real Diia cert.
# ABI (Task 4): SvdW c1..c4 removed (pinned in grumpkin_voprf). This is the
# blind-eval GATE: no DLEQ/Y/rinv here. INPUTS = cert/ECDSA/Diia fields, today,
# h0, h1, r_lo, r_hi. The circuit OUTPUTS the 13-word public layout:
#   today[0..8), then M.x[8] M.y[9] C_r[10] messageDigest_hi[11] messageDigest_lo[12],
# where C_r = commitR(r) (cross-proof shared-r binding, F2).
...
console.log("C_r (output)  =", "0x" + cR.toString(16).padStart(64, "0"));
