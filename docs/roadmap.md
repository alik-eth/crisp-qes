...
- **Coercion resistance (JCJ fake-credentials)** — *today's honest claim is **unlinkability, not coercion resistance***: the deterministic nullifier means a voter still holds the secret that could prove their own vote. Layered path: **unlinkability** (live — hides *who*) → **receipt-freeness** via the CRISP/FHE encrypted tally (no on-chain record of how any anonymous voter voted; see Track 1 LATER) → **JCJ fake credentials**, where a coerced voter surrenders a decoy and the ciphernode committee silently discards fake-credential votes at tally time under FHE (research-grade endgame — coercion resistance becomes a property of the tally, not something the voter must enforce). Re-vote/revoke until the deadline is a weak interim mitigation.
